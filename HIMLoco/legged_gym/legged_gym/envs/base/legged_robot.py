...
class LeggedRobot(BaseTask):
    ...
    def step(self, actions):
        ...
        if self.cfg.domain_rand.obs_delay:
            if not hasattr(self, 'delayed_obs_buf'):
                self.delayed_obs_buf = self.obs_buf.clone()
            
            # 生成 0 到 1 之间的随机混合系数 alpha
            # alpha 越接近 1，代表延迟越大（越接近上一帧）
            # alpha 越接近 0，代表延迟越小（越接近当前帧）
            alpha = torch.rand(self.num_envs, 1, device=self.device)
            
            # 线性插值混合上一帧和当前帧
            returned_obs_buf = alpha * self.delayed_obs_buf + (1.0 - alpha) * self.obs_buf

            # 控制命令和上一帧的动作覆盖回来
            returned_obs_buf[:, 0:3] = self.obs_buf[:, 0:3]
            returned_obs_buf[:, 33:45] = self.obs_buf[:, 33:45]
            
            self.delayed_obs_buf = self.obs_buf.clone()
        else:
            returned_obs_buf = self.obs_buf

        return returned_obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras, termination_ids, termination_priveleged_obs
    ...
    def _process_dof_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_joint_friction:
            ...
            for i in range(len(props)):
                # 乘以随机采样系数
                props["friction"][i] *= np.random.uniform(friction_range[0], friction_range[1])

        if self.cfg.domain_rand.randomize_joint_damping:
            damping_range = self.cfg.domain_rand.joint_damping_range
            for i in range(len(props)):
                # 乘以随机采样系数
                props["damping"][i] *= np.random.uniform(damping_range[0], damping_range[1])
    ...
    def _reset_dofs(self, env_ids):
        rand_offset = torch_rand_float(-0.1, 0.1, (len(env_ids), self.num_dof), device=self.device)
        self.dof_pos[env_ids] = torch.clip(self.default_dof_pos + rand_offset, self.dof_pos_limits[:, 0], self.dof_pos_limits[:, 1])
    ...
    def _create_envs(self):
        ...
        print("body names:", self.dof_names)
    ...
    def _reward_command_base_height(self):
        # Penalize base height away from target only when commanded to move
        base_height = self._get_base_heights()
        moving = torch.norm(self.commands[:, :2], dim=1) > 0.1
        height_error = torch.square(base_height - self.cfg.rewards.base_height_target)
        return torch.where(moving, height_error, torch.zeros_like(height_error))
    ...
    def _reward_zero_command_feet_lift(self):
        # Penalize lifted feet only while commanded to stand still.
        zero_command = (torch.norm(self.commands[:, :2], dim=1) < 0.1) & (torch.abs(self.commands[:, 2]) < 0.1)
        feet_lift = (self._get_feet_heights() - 0.05).clip(min=0.)
        max_lift = torch.max(feet_lift, dim=1).values
        penalty = (torch.sum(feet_lift, dim=1) + 2.0 * max_lift).clip(max=0.5)
        return torch.where(zero_command, penalty, torch.zeros_like(penalty))
    ...
    def _reward_hip_pos(self):
        flag = 1.*(torch.abs(self.commands[:,1]) == 0.0)
        return flag * torch.sum(torch.square(self.dof_pos[:, [0, 3, 6, 9]] - torch.zeros_like(self.dof_pos[:, [0, 3, 6, 9]])), dim=1)
    
    def _reward_feet_stumble(self):
        # 惩罚脚部撞击垂直表面（如台阶、障碍物）或严重的拖地摩擦
        
        # contact_forces 的维度通常是 (num_envs, num_bodies, 3) 包含x,y,z三个方向的力
        # self.feet_indices 是机器人四个脚在物理引擎中的索引
        
        # 1. 计算脚部受到的水平方向接触力的大小（XY平面）
        horizontal_forces = torch.norm(self.contact_forces[:, self.feet_indices, :2], dim=2)
        
        # 2. 计算脚部受到的垂直方向接触力的绝对值（Z轴）
        vertical_forces = torch.abs(self.contact_forces[:, self.feet_indices, 2])
        
        # 3. 核心判定逻辑：如果水平阻力超过了垂直支撑力的 5 倍，判定为“绊脚”或“严重蹭地”
        # 这个系数 5.0 是阈值，如果你的机器人在平地也疯狂触发惩罚，可以适当调大这个值 (比如 10.0)
        stumble = horizontal_forces > 5.0 * vertical_forces
        
        # 4. 只要有任何一只脚满足绊脚条件，该环境（env）当前 step 就输出 1.0 的惩罚信号
        # torch.any 会在脚的维度(dim=1)上进行求或操作
        return torch.any(stumble, dim=1).float()

    def _reward_feet_slip(self):
        # 惩罚脚部在接触地面时的水平滑动（解决拖地、蹭地、瘸腿问题）
        
        # 1. 判断脚是否踩在地上 (垂直接触力大于 1 认为触地)
        contact = self.contact_forces[:, self.feet_indices, 2] > 1.0
        
        # 2. 计算脚部在 XY 平面的水平速度
        foot_vel_xy = torch.norm(self.feet_vel[:, :, :2], dim=2)
        
        # 3. 如果触地，且有水平速度，则给予惩罚。速度越大，惩罚越重
        rew_slip = torch.sum(contact * foot_vel_xy, dim=1)
        
        return rew_slip
    
    def _reward_symmetry(self):
        # Trot symmetry for action order FL, FR, RL, RR with [hip roll, thigh pitch, calf pitch].
        safe_actions = torch.nan_to_num(self.actions, nan=0.0, posinf=0.0, neginf=0.0)
        actions = safe_actions.reshape(self.num_envs, 4, 3)
        hip = actions[:, :, 0]
        pitch_calf = actions[:, :, 1:3]

        # Diagonal legs should keep similar sagittal commands, while hip roll mirrors across sides.
        diag_pitch = 0.5 * (
            torch.mean(torch.square(pitch_calf[:, 0] - pitch_calf[:, 3]), dim=1) +
            torch.mean(torch.square(pitch_calf[:, 1] - pitch_calf[:, 2]), dim=1)
        )
        diag_hip = 0.5 * (
            torch.square(hip[:, 0] + hip[:, 3]) +
            torch.square(hip[:, 1] + hip[:, 2])
        )

        # Penalize persistent left/right or front/rear bias without forcing all legs into phase.
        left_pitch = 0.5 * (pitch_calf[:, 0] + pitch_calf[:, 2])
        right_pitch = 0.5 * (pitch_calf[:, 1] + pitch_calf[:, 3])
        front_pitch = 0.5 * (pitch_calf[:, 0] + pitch_calf[:, 1])
        rear_pitch = 0.5 * (pitch_calf[:, 2] + pitch_calf[:, 3])
        pair_pitch_bias = 0.5 * (
            torch.mean(torch.square(left_pitch - right_pitch), dim=1) +
            torch.mean(torch.square(front_pitch - rear_pitch), dim=1)
        )

        lr_hip_mirror = 0.5 * (
            torch.square(hip[:, 0] + hip[:, 1]) +
            torch.square(hip[:, 2] + hip[:, 3])
        )
        fb_hip_consistency = 0.5 * (
            torch.square(hip[:, 0] - hip[:, 2]) +
            torch.square(hip[:, 1] - hip[:, 3])
        )

        if hasattr(self, "last_actions") and self.last_actions is not None:
            safe_last_actions = torch.nan_to_num(self.last_actions, nan=0.0, posinf=0.0, neginf=0.0)
            delta = (safe_actions - safe_last_actions).reshape(self.num_envs, 4, 3)
            delta_hip = delta[:, :, 0]
            delta_pitch_calf = delta[:, :, 1:3]
            delta_diag_pitch = 0.5 * (
                torch.mean(torch.square(delta_pitch_calf[:, 0] - delta_pitch_calf[:, 3]), dim=1) +
                torch.mean(torch.square(delta_pitch_calf[:, 1] - delta_pitch_calf[:, 2]), dim=1)
            )
            delta_diag_hip = 0.5 * (
                torch.square(delta_hip[:, 0] + delta_hip[:, 3]) +
                torch.square(delta_hip[:, 1] + delta_hip[:, 2])
            )
            delta_symmetry = delta_diag_pitch + 0.5 * delta_diag_hip
        else:
            delta_symmetry = torch.zeros_like(diag_pitch)

        command_relax = torch.ones_like(diag_pitch)
        if hasattr(self, "commands"):
            cmd_limit = torch.finfo(diag_pitch.dtype).max / 4.0
            side_cmd = (
                torch.nan_to_num(torch.abs(self.commands[:, 1]), nan=0.0, posinf=cmd_limit, neginf=cmd_limit)
                if self.commands.shape[1] > 1 else torch.zeros_like(diag_pitch)
            )
            turn_cmd = (
                torch.nan_to_num(torch.abs(self.commands[:, 2]), nan=0.0, posinf=cmd_limit, neginf=cmd_limit)
                if self.commands.shape[1] > 2 else torch.zeros_like(diag_pitch)
            )
            command_relax = 1.0 / (1.0 + 1.5 * side_cmd + turn_cmd)

        diagonal_symmetry = diag_pitch + 0.5 * diag_hip
        pair_symmetry = 0.5 * pair_pitch_bias + 0.25 * lr_hip_mirror + 0.15 * fb_hip_consistency

        symmetry_penalty = (0.75 + 0.25 * command_relax) * diagonal_symmetry + command_relax * (pair_symmetry + 0.25 * delta_symmetry)
        penalty_limit = torch.finfo(symmetry_penalty.dtype).max / 4.0
        return torch.nan_to_num(symmetry_penalty, nan=0.0, posinf=penalty_limit, neginf=0.0).clamp_min(0.0)
